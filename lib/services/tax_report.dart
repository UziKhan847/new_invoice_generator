...
class TaxReportService {
  ...
  static Future<Uint8List> _buildPdf({
    ...
    final paidInvoices = invoices
        .where((i) => i.isPaid && i.issueDate.year == year)
        .toList()
      ..sort((a, b) => a.issueDate.compareTo(b.issueDate));

    final totalRevenue =
        paidInvoices.fold(0.0, (s, i) => s + i.taxableSubtotal);
    final totalTaxCollected =
        paidInvoices.fold(0.0, (s, i) => s + i.tax);
    final totalRevenueWithTax =
        paidInvoices.fold(0.0, (s, i) => s + i.total);
    ...
    final totalExpenses =
        yearExpenses.fold(0.0, (s, e) => s + e.amount);
    final totalInputTax =
        yearExpenses.fold(0.0, (s, e) => s + e.taxAmount);
      ...
      build: (ctx) => [
        ...
        pw.Container(
          ...
          child: pw.Column(
            ...
            children: [
              ...
              pw.Text(companyName,
                  style: pw.TextStyle(
                      color: PdfColors.white.shade(0.7), fontSize: 12)),
              pw.Text(
        ...
        pw.Row(children: [
          _summaryBox('Total Revenue\n(pre-tax)',
              '\$${totalRevenue.toStringAsFixed(2)}', PdfColors.green700),
          pw.SizedBox(width: 8),
          _summaryBox('HST/GST Collected',
              '\$${totalTaxCollected.toStringAsFixed(2)}', PdfColors.blue700),
          pw.SizedBox(width: 8),
          _summaryBox('Total Expenses',
              '\$${totalExpenses.toStringAsFixed(2)}', PdfColors.orange700),
          pw.SizedBox(width: 8),
          _summaryBox('Net Profit',
              '\$${netProfit.toStringAsFixed(2)}',
              netProfit >= 0 ? PdfColors.green700 : PdfColors.red700),
        ]),
        pw.SizedBox(height: 8),
        pw.Row(children: [
          _summaryBox('Input Tax Credits',
              '\$${totalInputTax.toStringAsFixed(2)}', PdfColors.teal700),
          pw.SizedBox(width: 8),
          _summaryBox('Net Tax Owing',
        ...
        if (paidInvoices.isNotEmpty) ...[
          ...
          pw.Table(
            ...
            children: [
              _tableHeader(['Date', 'Customer', 'Pre-tax', 'Tax (13%)', 'Total']),
              ...paidInvoices.map((inv) => _tableRow([
        ...
        if (yearExpenses.isNotEmpty) ...[
          ...
          pw.Table(
            ...
            children: [
              _tableHeader(['Date', 'Description', 'Category', 'Tax Paid', 'Amount']),
              ...yearExpenses.map((exp) => _tableRow([
