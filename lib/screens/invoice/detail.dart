              ...
              IconButton(
                ...
                onPressed: () async {
                  ...
                  if (context.mounted) {
                    ...
                    showEmailInvoiceDialog(context: context, invoice: invoice, logoUrl: logoUrl, company: company, customer: customer);
                  }
                },
              ),
                ...
                onPressed: () async {
                  ...
                  if (context.mounted) {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) =>
                            _PdfPreviewScreen(invoice: inv, company: company, customer: _resolveCustomer(ref, invoice.customerId)),
                      ),
                    );
                  }
                },
                ...
                onPressed: () async {
                  ...
                  if (context.mounted) {
                    await DownloadService.downloadInvoice(
                      ...
                      company: company,
                      customer: _resolveCustomer(ref, invoice.customerId),
                    );
                  }
                },
              ),
                                ...
                                if (invoice.isPrivate)
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: AppColors.of(context).purpleBg,
...
class _PdfPreviewScreen extends StatelessWidget {
  ...
  const _PdfPreviewScreen(
      {required this.invoice, this.company, this.customer});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              final bytes = await PdfService.buildPdfBytes(invoice, company: company, customer: customer);
      ...
      body: PdfPreview(
        ...
        build: (format) => PdfService.buildPdfBytes(invoice, company: company, customer: customer),
...
/// Resolve the live Customer for an invoice (by customerId) from the provider,
/// used as a fallback so the PDF shows full customer details even when the
/// invoice's own snapshot is empty. Returns null if not found.
